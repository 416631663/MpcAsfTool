...
#include "FreeImage.h"       // 如果你直接使用 FreeImage
#include "About.h"
#include "BatDialog.h"
#include "ResizeDialog.h"
#include <wx/dir.h>  // wxDir, wxDir::GetAllFiles, wxDIR_FILES
#include <map>       // std::map
...
BEGIN_EVENT_TABLE(MpcAsfTool,wxFrame)
    ...
    EVT_MENU(ID_EXPORT_TO_TGA, MpcAsfTool::OnExportToTga)// --------------- [新增] 绑定我们 ID_EXPORT_TO_TGA 的回调函数 ---------------
    ...
    //EVT_MENU(ID_TGA_TO_SPR, MpcAsfTool::OnFileExplorerWindow)// --------------- [新增] 绑定我们 ID_TGA_TO_SPR 的回调函数 ---------------
...
MpcAsfTool::MpcAsfTool(wxWindow* parent,wxWindowID id,const wxPoint& pos,const wxSize& size)
{
    ...
    Create(parent, wxID_ANY, _T("JX SPR编辑器"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_FRAME_STYLE, _T("wxID_ANY"));
    SetClientSize(wxSize(978,737));
    SetForegroundColour(wxSystemSettings::GetColour(wxSYS_COLOUR_ACTIVECAPTION));
    ...
    BoxSizer28->Add(StaticText_CurrentFile, 1, wxALL|wxEXPAND, 5);
    ...
    BoxSizer1->Add(BoxSizer28, 0, wxALL|wxEXPAND, 5);
    ...
    StaticBitmap_Show = new wxStaticBitmap(ScrolledWindow_BmpShow, ID_STATICBITMAP1, showbitmap, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE, _T("ID_STATICBITMAP1"));
    ...
    BoxSizer3->Add(ScrolledWindow_BmpShow, 1, wxALL|wxEXPAND, 0);
    ...
    BoxSizer3->Add(BoxSizer19, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer3->Add(BoxSizer29, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer9, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer13, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer14, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer10, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer11, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer12, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer15, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer24, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer16, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer17, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer18, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer27, 0, wxALL|wxEXPAND, 0);
    ...
    BoxSizer4->Add(BoxSizer20, 0, wxALL|wxEXPAND, 5);
    ...
    BoxSizer4->Add(BoxSizer23, 0, wxALL|wxEXPAND, 0);
    BoxSizer25->Add(BoxSizer4, 9, wxEXPAND, 5);
    ...
    BoxSizer21->Add(ScrolledWindow_Setting, 0, wxALL|wxEXPAND, 5);
    BoxSizer2->Add(BoxSizer21, 0, wxALL|wxEXPAND, 5);
    BoxSizer1->Add(BoxSizer2, 10, wxALL|wxEXPAND, 5);
	...
	menu_file->Append(ID_EXPORT_TO_TGA, wxT("导出到 TGA..."));     	//--新增tga导出
	...
	menu_file->Enable(ID_EXPORT_TO_TGA, false);
    ...
    // 在 MainFrame 的构造函数中添加菜单项
    wxMenu* menuTgaToSpr = new wxMenu();
    menuTgaToSpr->Append(ID_TGA_TO_SPR, wxT("TGA 转 SPR"));
    ...
    MenuBar_MpcAsfTool->Append(menuTgaToSpr, wxT("转换")); //新增转换
...
void MpcAsfTool::OnExportToTga(wxCommandEvent &event)
// 这个函数演示如何一次导出所有未删除帧到多张 TGA 文件
{
    // 1) 让用户选一个文件夹，而不是一个单独文件名
    wxDirDialog dlg(
        this,
        wxT("选择一个文件夹来存放导出的 TGA 图像"),
        wxEmptyString,
    ...
    // 获取“未删除”帧的总数
    // 注意：如果你自己的函数叫 GetFrameCounts() 或者别的，就替换成正确的
    // 下面假设 manager.GetFrameCounts() 返回“未删除”帧的数量
    // 也有人写 manager.GetFrameCount()；请以你自己的 WorkManager 接口为准
    unsigned long frameCount = manager.GetFrameCounts();

    // 如果没有帧，就直接提示并返回
    if (frameCount == 0)
    {
        wxMessageBox(wxT("没有可导出的帧。"), wxT("提示"));
        return;
    }

    // 2) 遍历所有帧，把每帧都保存成一个 TGA 文件
    //    假设第 i 帧的数据由 GetUndeletedGlobalizedFrameData(i) 获取
    //    如果你的代码里是别的名字或需要先判断是否 isdeleted，请自行调整
    for (unsigned long i = 0; i < frameCount; i++)
    {
        FILOCRGBQUAD* data = manager.GetUndeletedGlobalizedFrameData(i);
        if (!data)
        {
            // 如果这个帧获取失败，就跳过或继续
            // 也可以考虑给用户提示
        ...
        // 读取全局宽高
        long w = manager.GetGlobalWidth();
        long h = manager.GetGlobalHeight();

        // 转成 32位 BGRA 的 FIBITMAP
        ...
        {
            // 如果转换失败，给个提示
            wxLogError(wxT("第 %lu 帧转换成位图失败，无法导出。"), i);
            continue;
        }

        // 3) 生成文件名，比如 “frame0001.tga”, “frame0002.tga”
        //    你也可以用别的命名方式
        wxString filename = wxString::Format("frame_%04lu.tga", i + 1);
        wxString fullpath = folderPath + "\\" + filename; // 拼出完整路径

        // 4) 保存 TGA 文件
        BOOL ok = FreeImage_Save(FIF_TARGA, bitmap, fullpath.mb_str(wxConvUTF8), TARGA_DEFAULT);
        FreeImage_Unload(bitmap);

        // 可选：可以打印/记录下是否成功
        if (!ok)
        {
          wxLogError(wxT("保存 %s 时出错！"), fullpath);
        }
    }
...
void MpcAsfTool::EnableCtrl()
{
	...
	filemenu->Enable(ID_EXPORT_TO_TGA, true);
